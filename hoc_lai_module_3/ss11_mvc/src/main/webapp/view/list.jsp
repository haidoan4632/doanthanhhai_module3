...
<table class="table table-striped ms-5" style="width: 95%" border="1">
    ...
    <c:forEach var="book" items="${bookList}">
        <tr>
            ...
            <td>${book.category}</td>
            <td>
                <a href="user?action=update&id=${book.id}">Update</a>
            </td>
            <td>
                <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#modelId${book.id}">
                    Xóa
                </button>
            </td>
            <td><a href="/book?action=searchByTitle&title=${book.title}">
                <button class="btn btn-success btn-sm">Tìm kiếm theo title</button>
            </a></td>
        </tr>
        <!-- Modal -->
